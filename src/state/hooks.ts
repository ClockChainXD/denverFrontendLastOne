...
export const usePollFarmsData = (includeArchive = false) => {
  ...
  useEffect(() => {
    ...
    dispatch<any>(fetchFarmsPublicDataAsync(pids))

    if (account) {
      dispatch<any>(fetchFarmUserDataAsync({ account, pids }))
    }
...
export const usePollCoreFarmData = () => {
  ...
  useEffect(() => {
    dispatch<any>(fetchFarmsPublicDataAsync([251, 252]))
...
export const usePollBlockNumber = () => {
  ...
  useEffect(() => {
    const interval = setInterval(async () => {
      ...
      dispatch<any>(setBlock(blockNumber))
    }, 6000)
...
export const useFetchPublicPoolsData = () => {
  ...
  useEffect(() => {
    const fetchPoolsPublicData = async () => {
      const blockNumber = await simpleRpcProvider.getBlockNumber()
      dispatch<any>(fetchPoolsPublicDataAsync(blockNumber))
    }

    fetchPoolsPublicData()
    dispatch<any>(fetchPoolsStakingLimitsAsync())
...
export const useFetchCakeVault = () => {
  ...
  useEffect(() => {
    dispatch<any>(fetchCakeVaultPublicData())
  }, [dispatch, fastRefresh])

  useEffect(() => {
    dispatch<any>(fetchCakeVaultUserData({ account }))
  }, [dispatch, fastRefresh, account])

  useEffect(() => {
    dispatch<any>(fetchCakeVaultFees())
  }, [dispatch])
}
...
export const useGetCollectibles = () => {
  ...
  useEffect(() => {
    ...
    if (!isInitialized) {
      dispatch<any>(fetchWalletNfts(account))
    }
